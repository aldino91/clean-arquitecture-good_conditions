            ...
            if (xhr.status === 200) {
                ...
                if (data.error) {
                    ...
                    document.getElementById('itemDescription').innerHTML =
                        ...
                        '<button id="ver_docs" onclick="window.location.href=\'' + window.location.origin + '/documento/menu-documentos/' + data.id + '\'">Ver Documentos</button>' +
                        '<button id="nuevo_mensaje onclick="window.location.href=\'' + window.location.origin + '/credito/menu-mensajes/' + data.id + '\'">Nuevo mensaje</button>' +
                        '<button id="ver_prop"  data-bs-toggle="modal" data-bs-target="#PopupPropuestas">Ver propuestas</button>' +
                        '<button id="subir_docs" onclick="window.location.href=\'' + window.location.origin + '/documento/subida-cliente/' + data.id + '\'">Subir documentos</button>' +
                        '</div>';
                }
            ...
            if (xhr.status === 200) {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    data.forEach(function (credito) {
                        ...
                        contenido +=
                            `<button class="button-sin-prop" onclick="location.href = '${window.location.origin}/propuesta/crear-propuesta/${credito.id}'">` +
                            'ID Cliente: ' + credito.cliente_id + '<br>' +
                            'Crédito: ' + credito.valor_crédito + ' Eur ' + '<br>' +
                            'Tipo: ' + credito.opción_financiera + '<br>' +
                            'Balance: ' + balance + ' Eur'
                        '</button>';
                    });
            ...
            if (xhr.status === 200) {
                ...
                if (data.error) {
                    ...
                    document.getElementById('itemDescription').innerHTML =
                        ...
                        '<button id="ver_docs" onclick="window.location.href=\'' + window.location.origin + '/documento/menu-documentos/' + data.id + '\'">Ver Documentos</button>' +
                        '<button id="nuevo_mensaje"  onclick="window.location.href=\'' + window.location.origin + '/credito/menu-mensajes/' + data.id + '\'">Nuevo mensaje</button>' +
                        '<button id="ver_propuestas" onclick="VerPropuestas( {{credito.id}} )"  data-bs-toggle="modal" data-bs-target="#PopupPropuestas">Ver propuestas</button>'
                    '</div>';
                }
            } else if (xhr.status === 401 || xhr.status === 403) {
                // Si el código de estado es 401 (No Autorizado) o 403 (Prohibido), redirijo al usuario a la página de login
